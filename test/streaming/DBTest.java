...
//    @Test
    ...
    public void req13() {
        EntityManager em = Persistence.createEntityManagerFactory("StreamingPU").createEntityManager();
        List<Object[]> table = em.createQuery("SELECT r,COUNT(f) FROM Realisateur r JOIN r.films f GROUP BY r ORDER by COUNT(f) DESC ").getResultList();
        System.out.println(table);
        for (Object[] tabObj : table) {
            Realisateur r = (Realisateur) tabObj[0];
            Long NbrFilm = (Long) tabObj[1];
            System.out.println( "Prenom= "+ r.GetPrenom()+"; Nom =" + r.GetNom()+"; NbrFilm = " + NbrFilm );
        }
    }
}
